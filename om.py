...
while True:
	...
	if choice == 1:
		See_rating()
         ...
         db1 = sqlite3.connect('dbase1')
         print(pd.read_sql_query("select res_name from restaurant",db1))
         ...
         conn	=	 sqlite3.connect('dbase1')
         curs = conn.cursor()
         curs.execute("INSERT INTO customer3 values (?,?,?)", (Name,new_rating,comment))
         print(pd.read_sql_query("select * from customer3",conn))
